...
class POExtracts(BasePage):
    ...
    saxParser.parse(TestResource.TestData.GlobalParameter.object_file)
    ...
    def click_delete_button(self,patt_name):
        row = len(self.driver.find_elements_by_xpath("//*[@id='existingExtractsTab']/table/tbody/tr"))
        i = row
        while i > 0:
            extract = self.driver.find_element_by_xpath("//*[@id='existingExtractsTab']/table/tbody/tr[%s]/td[4]/span" % i)
            if re.match(patt_name,extract.text):
                self.driver.find_element_by_xpath("//*[@id='existingExtractsTab']/table/tbody/tr[%s]/td[3]/div/button/span" % i).click()
